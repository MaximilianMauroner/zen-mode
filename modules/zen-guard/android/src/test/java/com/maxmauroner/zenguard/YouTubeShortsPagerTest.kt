...
class YouTubeShortsPagerTest {
  ...
  @Test fun unbaselinedGestureFailsOpenEvenWhenTheFirstCallbackLooksDirectional() {
    val detector = YouTubeShortsPager()
    val pager = "com.google.android.youtube:id/reel_recycler"
    assertNull(detector.stablePageIndex(true, pager, 6, 7, 1))
    assertNull(detector.stablePageIndex(true, pager, 7, 7, 1))
    assertNull(detector.stablePageIndex(true, pager, 6, 7, 1))
    assertEquals(6, detector.stablePageIndex(true, pager, 6, 6, 0))
  }

  @Test fun repeatedPartialCallbacksCannotReplaceTheFirstGestureOrigin() {
    val detector = YouTubeShortsPager()
    val pager = "com.google.android.youtube:id/reel_recycler"
    assertNull(detector.stablePageIndex(true, pager, 6, 6, 0))
    assertNull(detector.stablePageIndex(true, pager, 6, 7, 1))
    assertNull(detector.stablePageIndex(true, pager, 6, 7, 1))
    assertNull(detector.stablePageIndex(true, pager, 6, 6, 0))
  }
